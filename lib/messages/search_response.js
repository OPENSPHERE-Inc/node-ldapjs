...
SearchResponse.prototype.send = function (entry, nofiltering) {
  ...
  if (entry instanceof SearchEntry || entry instanceof SearchReference) {
    ...
    Object.keys(entry.attributes).forEach(function (a) {
      // hanatyan@osi: Attribute names should be compare case insensitively
      if (!nofiltering && a.length && a[0] === '_') {
        savedAttrs[a] = entry.attributes[a]
        delete entry.attributes[a]
      } else if (!nofiltering && self.notAttributes.find((attr) => attr.toLowerCase() === a.toLowerCase())) {
        savedAttrs[a] = entry.attributes[a]
        delete entry.attributes[a]
      } else if (all) {
        // do nothing
      } else if (self.attributes.length && !self.attributes.find((attr) => attr.toLowerCase() === a.toLowerCase())) {
        savedAttrs[a] = entry.attributes[a]
        delete entry.attributes[a]
      }
    })
